...
class AppTests(unittest.TestCase):
    ...
    CMD_PREFIX = 'python -m pyment.pymentapp %s'
    ...
    PATCH_PREFIX = '# Patch generated by Pyment v%s' % pyment.pyment.__version__
    ...
    def runPymentAppAndAssertIsExpected(self,
        ...
        def assert_output(cmd_to_run, what, got, expected):
            ...
            if isinstance(expected, self.RE_TYPE):
                msg = "Test failed for cmd %s\n%s was expected to match the regex:\n%s\nBut this was the output:\n%r\n" % \
                ...
                #  %r is used instead of %s to make it easier to see newlines and spaces if there's a difference
                msg = "Test failed for cmd %s\n%s was expected to be:\n%r\nBut this was the output:\n%r\n" % \
                      (cmd_to_run, what, expected, got)
                assert got == expected, msg

        cmd_to_run = self.CMD_PREFIX % cmd_args

        if output_format:
            cmd_to_run = '%s --output %s ' % (cmd_to_run, output_format)
    ...
    def runPymentAppWithAFileAndAssertIsExpected(self,
        ...
        try:
            ...
            cmd_args = "%s %s" % (cmd_args, input_filename)

            if overwrite_mode:
                cmd_args = "%s -w " % cmd_args
            ...
            if overwrite_mode:
                ...
                output = re.sub(r'/%s$' % os.path.basename(input_filename), r'/-', output, flags=re.MULTILINE)
